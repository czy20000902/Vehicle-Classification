...
parser.add_argument('--data_url', default='/home/chenzhaoyang/dataset', type=str, help='IMAGENET')
parser.add_argument('--train_url', default='./resnet18', type=str, help='Path to model and saved Cov Matrix (default: ./)')
...
def main():
    ...
    # checkpoint_dir = os.path.join(args.train_url, './resnet18/model_state_best.pt')
    # checkpoint_dir = './resnet18/model_state_best.pt'
    ...
    model.load_state_dict(torch.load('./resnet18/model_state_best.pt'))
    fc.load_state_dict(torch.load('./resnet18/fc_state_best.pt'))
    feature_num = 512
    print('Number of final features: {}'.format(int(512)))
    print('Number of model parameters: {}'.format(sum([p.data.nelement() for p in model.parameters()])))

    model = model.cuda()
    criterion_isda = ISDALoss(feature_num, 13).cuda()
    ...
    valdir = os.path.join(args.data_url, 'test/hard')
    ...
    for epoch in range(args.epochs):
        ...
        train(train_loader, model, fc, criterion_isda, optimizer, epoch, args)
        ...
        for i in range(var.shape[0]):
            ...
            if not exists(os.path.join(args.train_url, 'Covariance/')):
                makedirs(args.train_url+'Covariance/')
            f = open(csv_name, 'w')
...
def train(train_loader, model, fc, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        loss, output = criterion(model, fc, images, target, args.lambda_0 * (epoch / args.epochs))

...
def test(epoch, batch_size, store_name, resume=False, model_path=None, backbone="resnet18"):
    ...
    if exists('./isda_model/' + backbone + '/model_state_best.pt'):
        model.load_state_dict(torch.load('./isda_model/' + backbone + '/model_state_best.pt'))
    else:
        print('building new model.')
    if exists('./isda_model/' + backbone + '/fc_state_best.pt'):
        fc.load_state_dict(torch.load('./isda_model/' + backbone + '/fc_state_best.pt'))
    else:
    ...
    with open(backbone + '/results_test.txt', 'a') as file:
        file.write('Iteration %d, test_acc = %.5f, test_loss = %.6f\n' % (
            epoch, test_acc, test_loss))
    print("Average Acc: %.4f" % (test_acc))

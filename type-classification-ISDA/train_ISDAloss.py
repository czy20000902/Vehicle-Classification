...
def test(model, fc, criterion, batch_size, input_size):
    ...
    test_data = torchvision.datasets.ImageFolder(root='/home/chenzhaoyang/data/cls_14/tests/head', transform=transform_test)
...
def train(epoch, batch_size, store_name, model_path=None, use_best=1, learning_rate=0.01):
    ...
    try:
        ...
        os.makedirs(exp_dir)
    store_name = './isda_model/resnet18'
    transform_train = transforms.Compose([transforms.Resize((input_size, input_size)),
                                          transforms.RandomHorizontalFlip(),
                                          transforms.ToTensor(),
                                          transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))])
    train_data = torchvision.datasets.ImageFolder(root='/home/chenzhaoyang/data/cls_14/combine_new', transform=transform_train)
    ...
    if use_best:
        ...
        if exists('./' + store_name + '/model_state.pt'):
            print('loading model_state.pt.')
            model.load_state_dict(torch.load('./' + store_name + '/model_state.pt'))
            # model =torch.load('./' + store_name + '/model_state.pth')
        else:
            print('building fc_state.pt.')
        if exists('./' + store_name + '/fc_state.pt'):
            print('loading fc_state.pt.')
            fc.load_state_dict(torch.load('./' + store_name + '/fc_state.pt'))
        else:
            print('building new fc.')
    # GPU
